...
const MenuItem: React.FC<MenuItemProps> = ({
  ...
  const content = (
    <motion.div
      whileTap={{ scale: 0.98 }}
      className={`flex items-center gap-4 px-4 py-4 active:bg-accent ${
...
const MyPage: React.FC = () => {
    ...
    return (
      <div className="min-h-screen flex flex-col bg-background">
        <div className="flex-1 flex flex-col items-center justify-center px-4 py-12 bg-card">
          ...
          <h1 className="text-xl font-bold text-text-main mb-2">로그인이 필요합니다</h1>
          ...
          <div className="flex gap-3 w-full max-w-xs">
            <Link
              to="/login"
              className="flex-1 bg-primary text-white font-bold py-3 rounded-xl text-center"
            >
              로그인
            </Link>
            <Link
              to="/signup"
              className="flex-1 bg-accent border border-border text-text-main font-bold py-3 rounded-xl text-center"
            >
              회원가입
            </Link>
          </div>
  ...
  return (
    <div className="min-h-screen bg-surface">
      ...
      <div className="px-4 py-6 border-b border-border bg-card">
        ...
        <div className="grid grid-cols-3 gap-4 mt-6">
          <Link to="/favorites" className="bg-accent rounded-xl p-4 text-center">
            ...
            <p className="text-lg font-bold text-text-main">{state.favorites.length}</p>
          ...
          <Link to="/mypage/views" className="bg-accent rounded-xl p-4 text-center">
            ...
            <p className="text-lg font-bold text-text-main">{state.recentViews.length}</p>
      ...
      {/* App Version */}
      <div className="text-center py-4 text-xs text-text-light">
        LunaAlba v1.0.0
      </div>
    </div>
  );
