...
const RichTextEditor: React.FC<RichTextEditorProps> = ({ value, onChange, placeholder: _placeholder }) => {
    ...
    const ToolbarButton = ({
        ...
        <button
            ...
            className={`p-1.5 rounded hover:bg-gray-200 transition-colors ${active ? 'text-primary bg-gray-200' : 'text-gray-600'}`}
        >
            <Icon size={16} />
        </button>
    );
    ...
    return (
        <div className={`border rounded-lg overflow-hidden transition-colors bg-white text-black ${isFocused ? 'border-primary ring-2 ring-primary/20' : 'border-gray-300'}`}>
            {/* Toolbar */}
