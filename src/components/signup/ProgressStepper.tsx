    ...
    return (
        <div className="mb-8">
            <div className="flex items-center justify-between relative">
                {/* Progress Bar */}
                <div className="absolute top-5 left-0 w-full h-1 bg-white/10 -z-10" />
                <div
                            ...
                            <div
                                className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all ${isComplete
                                        ...
                                        : isCurrent
                                            ...
                                            : 'border-white/20 bg-background text-text-muted'
                                    }`}
                            >
                                {isComplete ? <Check size={20} /> : stepNum}
                            </div>
                            <span className={`text-xs text-center ${isCurrent ? 'text-white font-bold' : 'text-text-muted'}`}>
                                {label}
                            </span>
                        </div>
                    );
