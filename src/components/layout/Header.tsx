...
const Header: React.FC = () => {
    ...
    return (
        <header className="sticky top-0 z-50 bg-background/95 backdrop-blur-sm border-b border-border">
            ...
            <div className="container mx-auto px-4 h-16 flex items-center justify-between">
                ...
                <Link to="/" className="flex items-center gap-2">
                    <span className="text-2xl font-bold text-primary">
                        Luna<span className="text-text-main">Alba</span>
                    </span>
                </Link>
                ...
                <div className="hidden md:flex flex-1 max-w-xl mx-8 relative">
                    <input
                        ...
                        className="w-full h-10 pl-4 pr-10 rounded-full bg-surface border border-border focus:border-primary focus:outline-none text-text-main placeholder-text-muted transition-colors"
                    />
                ...
                <div className="flex items-center gap-4">
                    {user ? (
                        ...
                        <div className="relative">
                            <button
                                ...
                                className="hidden md:flex items-center gap-2 text-text-main hover:text-primary transition-colors"
                            >
                                ...
                                <div className="absolute right-0 top-full mt-2 w-48 bg-card border border-border rounded-lg shadow-xl overflow-hidden">
                                    ...
                                    {user.type === 'advertiser' && (
                                        <Link
                                            ...
                                            className="flex items-center gap-2 px-3 py-2 text-sm text-text-muted hover:text-text-main hover:bg-surface"
                                        >
                        ...
                        /* Logged Out State */
                        <>
                            <Link to="/login" className="hidden md:flex items-center gap-1 text-text-muted hover:text-text-main transition-colors">
                                <LogIn size={18} />
                                <span className="text-sm">로그인</span>
                            </Link>
                            <Link to="/signup" className="hidden md:flex items-center gap-1 text-text-muted hover:text-text-main transition-colors">
                                <User size={18} />
                    ...
                    {isAdvertiser() && (
                        <Link
                            to="/advertiser/dashboard"
                            className="hidden md:flex items-center gap-1 text-text-muted hover:text-text-main transition-colors mr-2"
                        >
                    ...
                    <Link
                        to="/post-ad"
                        className="flex items-center gap-2 bg-primary hover:bg-primary-hover text-white font-bold px-4 py-2 rounded-full transition-colors"
                    >
                        <PenSquare size={18} />
                        <span>광고등록</span>
                    </Link>
                    ...
                    <div className="container mx-auto px-4 py-4 space-y-4">
                        {user ? (
                            ...
                            <div className="flex gap-3 pb-4 border-b border-border">
                                <Link to="/login" onClick={() => setShowMobileMenu(false)} className="flex-1 py-2 text-center text-sm text-text-main bg-surface rounded-lg border border-border">
                                    로그인
                                </Link>
                                <Link to="/signup" onClick={() => setShowMobileMenu(false)} className="flex-1 py-2 text-center text-sm text-white bg-primary rounded-lg font-bold">
                                    회원가입
                                </Link>
                            </div>
                        )}
                        <nav className="space-y-1">
                            {[
                                ...
                                <Link
                                    ...
                                    className="block py-2 text-text-muted hover:text-text-main"
                                >
                                    {item.name}
                                </Link>
                            ))}
            ...
            <nav className="hidden md:block border-t border-border bg-surface/50">
